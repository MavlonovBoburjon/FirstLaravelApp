                    ...
                    @auth()
                        @canany(['update','delete'],$post)
                            <div class="float-right d-flex mb-2">
                                ...
                                <form action="{{ route('posts.destroy',['post' => $post->id]) }}" method="POST" onsubmit="return confirm('Rosdan ham o\'chirishni hohlaysizmi?');">
                                    @csrf
                                    @method('DELETE')
                        ...
                        @foreach($post->comments as $comment)
                            <div class="media mb-4">
                                ...
                                <div class="media-body">
                                    <h6>{{ $comment->user->name }}<small><i>{{$comment->created_at->format('d-m-Y H:00')}}</i></small></h6>
                                    <p>{{ $comment->body }}</p>
...
{{--                            </div>--}}
                ...
                <div class="col-lg-4 mt-5 mt-lg-0">
                    <div class="d-flex flex-column text-center bg-secondary rounded mb-5 py-5 px-4">
                        ...
                        <p class="text-white m-0 text-break">Conset elitr erat vero dolor ipsum et diam, eos dolor lorem ipsum,
                            ipsum
                    ...
                    <div class="mb-5">
                        <h3 class="mb-4 section-title">Categoriyalar</h3>
                    ...
                    <div class="mb-5">
                        ...
                        @foreach($recently_posts as $recently_post)
                            <div class="d-flex align-items-center border-bottom mb-3 pb-3">
                            <img class="img-fluid rounded" src="{{asset('storage/'.$recently_post->photo)}}" style="width: 80px; height: 80px; object-fit: cover;" alt="">
                            <div class="d-flex flex-column pl-3">
                                <a class="text-dark mb-2" href="{{route( 'posts.show' , [ 'post'=> $recently_post->id ])}}">{{ $recently_post->title }}</a>
                                <div class="d-flex">
                                    @foreach($recently_post->tags as $tag)
                                        <small><a class="text-secondary text-uppercase font-weight-medium" href="">{{$tag->name}}</a></small>
